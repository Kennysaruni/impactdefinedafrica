        ...
        <footer className='bg-gray-800 text-white ' id='contact'> {/* Added font-['Inter'] for consistency */}
            <div className='max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8'> {/* Changed container to max-w-7xl and added responsive padding */}
                <div className='grid grid-cols-1 md:grid-cols-3 gap-8 text-center md:text-left'> {/* Responsive grid, text alignment for mobile vs desktop */}
                    {/* Brand Info */}
                    <div>
                        <h3 className='text-xl font-bold mb-4'>Impact Defined Africa</h3> {/* Increased font size and weight */}
                        <p className='text-gray-400 text-base'>For A Better Future</p> {/* Consistent text size */}
                    </div>

                    {/* Contact Info */}
                    <div>
                        <h3 className='text-xl font-bold mb-4'>Contact Us</h3> {/* Increased font size and weight */}
                        <ul className='space-y-3 text-gray-400 text-base'> {/* Increased space-y for better readability */}
                            <li className='flex items-center justify-center md:justify-start'> {/* Centered on mobile, left-aligned on desktop */}
                                <span className='material-icons mr-2 text-xl'>email</span> {/* Increased icon size */}
                                info@impactdefinedafrica.org
                            </li>
                            <li className='flex items-center justify-center md:justify-start'> {/* Centered on mobile, left-aligned on desktop */}
                                <span className='material-icons mr-2 text-xl'>phone</span> {/* Increased icon size */}
                                +1123 456 789
                            </li>
                            <li className='flex items-center justify-center md:justify-start'> {/* Centered on mobile, left-aligned on desktop */}
                                <span className='material-icons mr-2 text-xl'>location_on</span> {/* Increased icon size */}
                    ...
                    <div>
                        <h3 className='text-xl font-bold mb-4'>Follow Us</h3> {/* Increased font size and weight */}
                        <div className='flex justify-center md:justify-start space-x-4 items-center'> {/* Social media items in a row, centered on mobile, left-aligned on desktop */}
                            <a href="#" className='text-gray-400 hover:text-white transition-colors duration-200' aria-label="Facebook">
                                <svg aria-hidden="true" className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"> {/* Increased icon size */}
